...
outfile = 'figure_02.pdf'

covs = np.load('data/uncertainty_prediction/results/modern_stats.npy')
print(covs.shape)

results = pd.read_csv('data/uncertainty_prediction/results/modern_samples.csv', sep=',', header=0, index_col=0, converters={'in ellipse frequencies': pd.eval})
...
with plt.rc_context({**bundles.aistats2022(family="serif"), **axes.lines()}):
  ...
  for j, ax in enumerate(axes.ravel()):
    ...
    ax.scatter(*true_mean.T, c=palette[6], s=1)
    for conf in confs:
      h, e = get_ellipse(true_mean, true_cov, conf, palette[7])
      ax.add_patch(e)
      _, e = get_ellipse([0, 0], covs[j], conf, palette[6])
      ax.add_patch(e)
      ax.text(true_mean[0], true_mean[1] + h, rf'${{{conf}}}$', horizontalalignment='center',
     verticalalignment='center', fontsize=8)
  fig.supxlabel(r'$\tau_1 - \hat{\tau}_1$')
  fig.supylabel(r'$\tau_2 - \hat{\tau}_2$')
  fig.legend([c1, c2], [r'pred.', r'empir.'], handler_map={mpatches.Circle:HandlerPatch(patch_func=make_legend_ellipse),
                        }, loc='center left', bbox_to_anchor=(1, 0.5), title=r'$(\tau_{\mathcal{P}} - \hat{\tau}_{\mathcal{P}}) \sim \mathcal{N}$')
  plt.savefig(outfile)